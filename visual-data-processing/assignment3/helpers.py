...
def SSD_per_pixel(lhs, rhs):
    return SSD(lhs, rhs) / np.product(lhs.shape)
...
def imread_hdr(file):
    return bgr2rgb(cv2.imread(file, -1))

def imread_normalized_float(file, scale=1.0):
    image = bgr2rgb(cv2.imread(file))
    if scale != 1.0:
        image = resize_image(image, scale)
    return np.array(image / 255.0, dtype=np.float32)

def imread_normalized_float_grayscale(file, scale=1.0):
